...
static ssize_t get_index_size(GeoIP *gi, struct stat *buf) {
  ...
  if (segment != 0 && index_size / segment != (ssize_t)gi->record_length * 2) {
    return -1;
  }
...
static void _check_mtime(GeoIP *gi) {
  ...
  if (gi->flags & GEOIP_CHECK_CACHE) {
    ...
    if (0 != gettimeofday(&t, NULL)) {
      DEBUG_MSGF(gi->flags, "Error calling gettimeofday: %s\n",
                 strerror(errno));
      return;
    }
      ...
      if (buf.st_mtime != gi->mtime &&
        ...
        if (gi->flags & (GEOIP_MEMORY_CACHE | GEOIP_MMAP_CACHE)) {
          if (gi->cache && (gi->flags & GEOIP_MMAP_CACHE)) {
            ...
            if ((gi->cache = (unsigned char *)realloc(gi->cache,
                                                      buf.st_size)) == NULL) {
        ...
        if (gi->flags & GEOIP_MMAP_CACHE) {
#if defined(_WIN32)
          DEBUG_MSGF(gi->flags, "GEOIP_MMAP_CACHE is not supported on WIN32\n");
          gi->cache = 0;
          return;
#else
          gi->cache = mmap(NULL, buf.st_size, PROT_READ, MAP_PRIVATE,
                           fileno(gi->GeoIPDatabase), 0);
          if (gi->cache == MAP_FAILED) {

            DEBUG_MSGF(gi->flags, "Error remapping file %s when reloading\n",
                       gi->file_path);
          ...
          if (pread(fileno(gi->GeoIPDatabase), gi->cache, buf.st_size, 0) !=
              (ssize_t)buf.st_size) {
            DEBUG_MSGF(gi->flags, "Error reading file %s when reloading\n",
                       gi->file_path);
            return;
          }
        }
        ...
        if (gi->flags & GEOIP_INDEX_CACHE) {
          ...
          if (gi->index_cache != NULL) {
            if (pread(fileno(gi->GeoIPDatabase), gi->index_cache, idx_size,
                      0) != idx_size) {
              DEBUG_MSGF(gi->flags, "Error reading file %s where reloading\n",
                         gi->file_path);
              return;
            }
          }
...
unsigned int _GeoIP_seek_record_v6_gl(GeoIP *gi, geoipv6_t ipnum,
  ...
  for (depth = 127; depth >= 0; depth--) {
    ...
    if (gi->cache == NULL && gi->index_cache == NULL) {
      /* read from disk */
      if (pread(fno, stack_buffer, record_pair_length, (long)byte_offset) !=
          record_pair_length) {
        break;
      }
...
GeoIP *GeoIP_open(const char *filename, int flags) {
  ...
  if (flags & GEOIP_INDEX_CACHE) {
    ...
    if (gi->index_cache != NULL) {
      if (pread(fileno(gi->GeoIPDatabase), gi->index_cache, idx_size, 0) !=
          idx_size) {
...
char *GeoIP_database_info(GeoIP *gi) {
  printf("C code: calling GeoIP_database_info\n");
  int i;
  unsigned char buf[3];
  char *retval;
...
void GeoIP_assign_region_by_inetaddr_gl(GeoIP *gi, unsigned long inetaddr,
                                        GeoIPRegion *region, GeoIPLookup *gl) {
...
static GeoIPRegion *_get_region_gl(GeoIP *gi, unsigned long ipnum,
                                   GeoIPLookup *gl) {
  GeoIPRegion *region;
...
static GeoIPRegion *_get_region_v6_gl(GeoIP *gi, geoipv6_t ipnum,
                                      GeoIPLookup *gl) {
  GeoIPRegion *region;
