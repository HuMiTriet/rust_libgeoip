...
void usage(void) {
  fprintf(stderr, "Usage: geoiplookup [-h] [-?] [-d custom_dir] [-f "
...
void _say_range_by_ip(GeoIP *gi, uint32_t ipnum) {
  ...
  printf("  network:      %s - %s ::%ld\n", __num_to_addr_r(low, ipaddr),
         __num_to_addr_r(hi, tmp), last_nm);
  printf("  ipnum: %u\n", ipnum);
    ...
    if (GEOIP_DOMAIN_EDITION == i) {
      ...
      if (domain_name == NULL) {
        ...
        free((void *)domain_name);
               ...
               GEOIP_ACCURACYRADIUS_EDITION == i || GEOIP_ASNUM_EDITION == i ||
               GEOIP_USERTYPE_EDITION == i || GEOIP_REGISTRAR_EDITION == i ||
      ...
      if (NULL == gir) {
        ...
        printf(
            "%s: %s, %s, %s, %s, %s, %f, %f\n", GeoIPDBDescription[i],
            gir->country_code, _mk_NA(gir->region),
            _mk_NA(GeoIP_region_name_by_code(gir->country_code, gir->region)),
            _mk_NA(gir->city), _mk_NA(gir->postal_code), gir->latitude,
            gir->longitude);
        _say_range_by_ip(gi, ipnum);
