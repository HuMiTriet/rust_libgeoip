...
extern "C" fn GeoIP_open(
    ...
    println!("HERE with {file:#?} and mtime {}", file_metadata.st_mtime());
    ...
    if (flags as u32 & (GeoIPOptions_GEOIP_MEMORY_CACHE | GeoIPOptions_GEOIP_MMAP_CACHE)) != 0 {
        if flags as u32 & GeoIPOptions_GEOIP_MMAP_CACHE != 0 {
            #[cfg(unix)]
            {
                let cache = match unsafe { Mmap::map(&file) } {
                    Ok(c) => c,
                    Err(_) => return ptr::null_mut(),
                };
                gi.cache = Some(cache.to_vec());
            }
        } else {
    ...
    gi.last_mtime_check = 1000000000000;
